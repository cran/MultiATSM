...
(unspanned factors). A formal exposition of the model’s underlying features can be found in @Moura2025.
...
Due to peculiar features of JPS-based specifications (see @JoslinSingletonZhu2011), an efficient
...
-   A logical variable. Choose *TRUE* to impose the
    constraint that the largest eigenvalue under the $Q$-measure is
    strictly less than $1$ otherwise, it should be set to *FALSE*. While
...
must specify a logical variable set to *TRUE* if the computation of forward
...
3.  *Wish the estimation of the sigmas matrices*: this is a logical
    ...
    the Cholesky factorization matrices) and, *FALSE* otherwise. Since this
...
optimally selecting the model parameters via the `Optimization()` function.
Broadly, the unspanned risk framework from JPS (2014) (and, therefore,
all its multi-country extensions) follows a model parameterization similar to that proposed by @JoslinSingletonZhu2011. Specifically, it requires the estimation parameter set
consisting of 6 elements, namely the: 

1.    Risk-neutral long-run mean of the
short rate (*r0*);
...
4.    The standard deviation of the errors from the portfolios of yields observed with
error (*se*);
...
numerical estimation. This package provides standardized routines
to set effective initial values, facilitating the optimization process.


The *MultiATSM* package offers two optimization algorithms: *L-BFGS-B* and *Nelder-Mead*. By default, both algorithms are run sequentially twice
to ensure the model reaches the global optimum. Specifically in the
default bootstrap setup, only *Nelder-Mead* is used to save time.
...
(2024). See the **paper replication vignette** (included with this package) for a detailed reproduction. 
