...
framework from **(JPS, 2014)**. In essence, these models assume the
...
datasets can be accessed using the `LoadData()` function with the
...
factors**, such as economic growth and inflation. Similar to the bond
...
several general model inputs, namely:
...
its expected ($Exp_t^{(n)}$) and term premia ($TP_t^{(n)}$) components.
Technically: $$
...
If the user intends to generate confidence bands around the point
...
Once bond yields and economic time series data are gathered and model
...
dynamics under the $P-$measure. Finally, optimize the entire selected
ATSM, including the $Q-$measure parameters.
...
the features of the PC weight matrix as illustrated below:
...
factors, the user can simply use the `Spanned_Factors()` function as
follows:
...
discussion of GVAR models, see @ChudikPesaran2016.
...
numerical estimation. @LeSingleton2018 provides standardized routines
